...
__constant__ float weight_matrix[6000];
__constant__ float weight_matrix2[6000];
...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    */
    ...
    #define mask_4d(i3, i2, i1, i0) weight_matrix[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
...
{	    
    ...
    */
    ...
    #define out_4d(i3, i2, i1, i0) output[(i3) * (M * H_out * W_out) + (i2) * (H_out * W_out) + (i1) * (W_out) + i0]
    #define in_4d(i3, i2, i1, i0) input[(i3) * (C * H * W) + (i2) * (H * W) + (i1) * (W) + i0]
    #define mask_4d(i3, i2, i1, i0) weight_matrix2[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    int H_grid = ceil(H_out / (1.0*TILE_WIDTH));
        ...
        if (M < 24){
            ...
            cudaMemcpyToSymbol(weight_matrix, device_mask, sizeof(float) * M * C * K * K);
            ...
            cudaMemcpyToSymbol(weight_matrix2, device_mask, sizeof(float) * M * C * K * K);
            conv_forward_kernel<<<dimGrid, dimBlock>>>(device_output, device_input, device_mask, B, M, C, H, W, K, S);
        }
}
